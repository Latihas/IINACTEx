...
public class MultiSigScanner : IDisposable {
    ...
    public static nint Scan(nint baseAddress, int size, string signature) => Scan(baseAddress, size, signature, 1)[0];
    ...
    /// <param name="signature">The signature.</param>
    /// <returns>The real offset of the found signature.</returns>
    public nint ScanText(string signature) => ScanText(signature, 1)[0];
    ...
    public nint[] ScanText(string signature, int maxAddresses) {
        ...
        for (var i = 0; i < scanRet.Length; i++) {
            if (IsCopy)
    ...
    private void SetupSearchSpace() {
        ...
        Plugin.Log.Debug($"[MultiSigScanner] TextSectionOffset: 0x{TextSectionOffset:X}");
        Plugin.Log.Debug($"[MultiSigScanner] TextSectionSize: 0x{TextSectionSize:X}");
        Plugin.Log.Debug($"[MultiSigScanner] DataSectionOffset: 0x{DataSectionOffset:X}");
        Plugin.Log.Debug($"[MultiSigScanner] DataSectionSize: 0x{DataSectionSize:X}");
        Plugin.Log.Debug($"[MultiSigScanner] RDataSectionOffset: 0x{RDataSectionOffset:X}");
        Plugin.Log.Debug($"[MultiSigScanner] RDataSectionSize: 0x{RDataSectionSize:X}");
    }

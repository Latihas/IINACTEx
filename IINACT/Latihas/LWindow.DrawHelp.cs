...
public static partial class LWindow {
	...
	internal static void DrawHelpSettings() {
		...
		if (ImGui.CollapsingHeader("常见问题##常见问题", ImGuiTreeNodeFlags.DefaultOpen)) {
			...
			ImGui.TextWrapped("    : 多半是端口占用了，请改变Overlay Plugin的端口，然后重启游戏/插件；或是直接使用最后一栏的'历史记录'，直接走内存不走端口。");
			ImGui.Separator();
			ImGui.Text("4. 我可以多开吗");
	...
	private static void DrawTestTestSettings() {
		...
		ImGui.PopStyleColor(1);
		//==============================
		var configurationUseArrManager = Instance.Configuration.UseArrManager;
		if (ImGui.Checkbox("启用ArrManager(需要重启)", ref configurationUseArrManager)) {
			Instance.Configuration.UseArrManager = configurationUseArrManager;

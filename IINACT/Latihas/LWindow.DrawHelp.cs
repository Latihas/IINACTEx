...
public static partial class LWindow
{
    ...
    internal static void DrawHelpSettings()
    {
        ...
        ImGui.Separator();
        ImGui.Text("已知问题");
        ImGui.Text("插件在卸载的时候必定回收失败两个Hook。泄露的Hook大概率导致非重启游戏再次加载插件时会炸游戏。（更新也会卸载，建议更新完后重启游戏）");
        ImGui.Text("插件加载的第一次大概率会加载失败，重新加载即可。");
        ImGui.Text("上面这俩没定位到问题所在，可以提Pr之类的协助我修复。");
        ImGui.Separator();
        ImGui.Text("TODO");
        ImGui.Text("触发器生成器");
        ImGui.Text("完善脚本");

...
public partial class LWindow {
	internal class ConditionPanel {
		...
		public void Draw() {
			...
			if (ImGui.BeginChild("LeftConditionPanel", new Vector2(leftPanelWidth, 0), false, ImGuiWindowFlags.NoScrollbar)) {
			...
			ImGui.SameLine();
			if (ImGui.BeginChild("RightConditionPanel", new Vector2(rightPanelWidth, 0), true, ImGuiWindowFlags.NoScrollbar)) {
				ImGui.Text("条件属性");
				ImGui.Separator();
				if (Condition != null)
					switch (Condition) {

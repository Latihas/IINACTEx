...
internal class LineActorCastExtra : LineBaseSubMachina<LineActorCastExtra.ActorCastExtraPacket> {
    ...
    internal class ActorCastExtraPacket : MachinaPacketWrapper {
        public override string ToString(long epoch, uint ActorID) {
            var abilityId = Get<ushort>("ActionID");

            // for x/y/x, subtract 7FFF then divide by (2^15 - 1) / 100
            var x = FFXIVRepository.ConvertUInt16Coordinate(Get<ushort>("PosX"));
            // In-game uses Y as elevation and Z as north-south, but ACT convention is to use
            // Z as elevation and Y as north-south.
            var y = FFXIVRepository.ConvertUInt16Coordinate(Get<ushort>("PosZ"));
            var z = FFXIVRepository.ConvertUInt16Coordinate(Get<ushort>("PosY"));
            ...
            // a UInt16
            var h = FFXIVRepository.ConvertHeading(FFXIVRepository.InterpretFloatAsUInt16(Get<float>("Rotation")));

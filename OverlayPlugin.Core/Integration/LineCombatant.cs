...
public class LineCombatant : IDisposable {
    ...
    private int offsetHeaderActorID;
    private int offsetHeaderLoginUserID;
    ...
    private string FormatFieldChange(FieldInfo info, Combatant combatant, bool skipDefaultValues = false) {
        ...
        if (info.FieldType == typeof(float) || info.FieldType == typeof(double)) {
    ...
    private unsafe void MessageReceived(string id, long epoch, byte[] message) {
        fixed (byte* buffer = message) {
            var actorID = *(uint*)&buffer[offsetHeaderActorID];
            var loginID = *(uint*)&buffer[offsetHeaderLoginUserID];

...
/// <summary>
///     A byte offset by 0x1980 from the combatant's address that further describes the combatant if their ObjectType
///     is a Monster.
/// </summary>
...
public enum ObjectStatus : byte {
    /// <summary>
    ///     Indicates that a targetable PC, NPC, monster, minion, or object is at an effective distance of 97 or less from
    ///     the player.
    /// </summary>
    ...
    /// <summary>
    ///     Indicates that a targetable pet, chocobo, or sometimes a monster belonging to another one (add) is in the
    ///     vicinity.
    /// </summary>
...
public class Combatant {
    ...
    [NonSerialized] public byte RawEffectiveDistance;
    ...
    private float GetDistance(Combatant target) {
    ...
    public string DistanceString(Combatant target) => GetDistance(target).ToString("0.00");
    ...
    public string EffectiveDistanceString(Combatant target) {
        ...
        if (target.Radius == 0)
            return RawEffectiveDistance.ToString("0.00");

using System;
using System.Runtime.InteropServices;

namespace RainbowMage.OverlayPlugin.MemoryProcessors.EnmityHud;

public interface IEnmityHudMemory73 : IEnmityHudMemory;

public class EnmityHudMemory73(TinyIoCContainer container) : EnmityHudMemory(container, enmityHudPointerPath, enmityHudCountOffset, enmityHudEntryOffset, EnmityHudEntryMemory.Size), IEnmityHudMemory73 {
	// This signature might not be stable - If broken, this is looking for the location of `g_Component::GUI::AtkStage_Instance`
	// The current version looks at `Component::GUI::AtkComponentDropDownList.SetEnabledState` for its location
	private static readonly int[] enmityHudPointerPath = [0x30, 0x58, 0xA8, 0x20];

	// Offsets from the enmityHudAddress to find various enmity HUD data structures.
	private const int enmityHudCountOffset = 4;
	private const int enmityHudEntryOffset = 16;

	public override Version GetVersion() => new(7, 3);

	[StructLayout(LayoutKind.Explicit, Size = Size)]
	private struct EnmityHudEntryMemory {
		public const int Size = 24;

		[FieldOffset(0x00)] public uint Unknown01;

		[FieldOffset(0x04)] public uint HPPercent;

		[FieldOffset(0x08)] public uint EnmityPercent;

		[FieldOffset(0x0C)] public uint CastPercent;

		[FieldOffset(0x10)] public uint ID;

		[FieldOffset(0x14)] public uint Unknown02;
	}


	protected override unsafe EnmityHudEntry GetEnmityHudEntryFromBytes(byte[] source, int num = 0) {
		if (num is < 0 or > 8) throw new ArgumentException();

		fixed (byte* p = source) {
			var mem = *(EnmityHudEntryMemory*)&p[num * EnmityHudEntryMemory.Size];

			var enmityHudEntry = new EnmityHudEntry {
				Order = num,
				ID = mem.ID,
				HPPercent = mem.HPPercent > 100 ? 0 : mem.HPPercent,
				EnmityPercent = mem.EnmityPercent > 100 ? 0 : mem.EnmityPercent,
				CastPercent = mem.CastPercent > 100 ? 0 : mem.CastPercent
			};

			return enmityHudEntry;
		}
	}
}
...
public class DamageTypeData : IEquatable<DamageTypeData>, IComparable<DamageTypeData> {
				...
				for (var j = swingIndex; j < swingCount; j++) {
					...
					if (list.Count - 1 == list2.Count) {
						...
						for (var k = j; k < swingCount; k++) {
							...
							if (k + 1 != swingCount && (startTimesCount <= i + 1 || masterSwings[k + 1].Time < startTimes[i + 1])) continue;
							swingIndex = k - 1;
							break;
						}
	...
	public string GetColumnByName(string name) => !ColumnDefs.TryGetValue(name, out var col) ? string.Empty : col.GetCellData(this);
